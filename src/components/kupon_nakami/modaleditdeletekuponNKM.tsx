...
import { handleDeleteKuponNakami, handleDeleteVoucherNakami } from '../../service/fetchKuponNakami'
...
const ModalEditDeleteKuponNakami = ({ setShowVisibleModal, item }: props) => {
    const showKupon = item.Voucher === 0 || item.Kupon === null;
    const [confirmDelete, setConfirmDelete] = useState(false);
    const [notification, setNotification] = useState('');
    const navigationKupon = useNavigation<StackNavigationProp<RootStackParamList, 'EditKuponNakami'>>();
    const navigationVoucher = useNavigation<StackNavigationProp<RootStackParamList, 'EditVoucherNakami'>>();
                                ...
                                <View style={{ alignItems: 'center' }}>
                                    <TouchableOpacity
                                        onPress={() => {
                                            ...
                                            {
                                                showKupon
                                                    ? navigationKupon.reset({ index: 0, routes: [{ name: 'DetailKuponNakamiPage' }] })
                                                    : navigationVoucher.reset({ index: 0, routes: [{ name: 'DetailKuponNakamiPage' }] });
                                            }
                                        }}
                            ...
                            <>
                                ...
                                <View style={{ flexDirection: 'row', gap: 10, padding: 10, justifyContent:'center' }}>
                                    <TouchableOpacity
                                        ...
                                        onPress={() => {
                                            showKupon
                                                ? handleDeleteVoucherNakami(item.ID, item.Poin, item.Voucher, setNotification) 
                                                : handleDeleteKuponNakami(item.ID, item.Poin, item.Kupon, setNotification)
                                         }}>
                            ...
                            <View style={{ flexDirection: 'row', gap: 10 }}>
                                ...
                                <TouchableOpacity
                                    onPress={() => {
                                        setShowVisibleModal(false)
                                        if (showKupon) {
                                            navigationVoucher.navigate('EditVoucherNakami', { kuponnakamiId: item.ID, kuponnakamiPoin: item.Poin, kuponnakamiVoucher: item.Voucher });
                                        } else {
                                            navigationKupon.navigate('EditKuponNakami', { kuponnakamiId: item.ID, kuponnakamiPoin: item.Poin, kuponnakamiKupon: item.Kupon });
                                        }
                                    }}
                                    style={[styles.buttonEditDelete, { backgroundColor: showKupon ? Color.border : Color.primary }]}
                                >
...
export default ModalEditDeleteKuponNakami
