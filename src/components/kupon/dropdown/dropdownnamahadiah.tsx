...
import { FetchBarangPerPoin, FetchHadiahData, fetchPoinPerBarang } from '../../../service/fetchHadiah';
...
const DropdownNamaHadiah = ({namahadiah, setNamahadiah, hadiah, setHadiah}:DropdownNamaHadiahPorps) => {
    const [listNamaHadiah, setListNamaHadiah] = useState<Hadiah[]>([]);
    ...
    const handleDropdownaselect = (selectedValue: string) => {
        if (setNamahadiah !== undefined) {
            ...
            fetchPoinPerBarang(namahadiah, setHadiah, setModalType, setNotificationModal, setModal);   
        }
        toggleDropdown()
    }
    useEffect(() => {
        if (isNamaHadiahDropdownVisible) {
            setNamahadiah('')
            if (hadiah === undefined) {
                FetchHadiahData(setListNamaHadiah, setNotificationModal, setModal)
            }
            FetchBarangPerPoin(setListNamaHadiah, setNotificationModal, setModal, hadiah)
        }
