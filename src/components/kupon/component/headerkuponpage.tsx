import {
    View,
    Text,
    StyleSheet, TouchableOpacity, TextInput, NativeSyntheticEvent, TextInputSubmitEditingEventData, Animated
} from 'react-native'
import React, { useState } from 'react'
import MaterialIcons from 'react-native-vector-icons/MaterialIcons'
import { handleRefreshList, handleSearchKupon } from '../../../service/fetchKupon';
...
const HeaderKuponPage = ({ searchKuponQuery, setSearchKuponQuery, setKuponData, showSearch, setShowSearch, setGetError, setGetModalError }: props) => {
    ...
    const onSubmitEditing = async (event: NativeSyntheticEvent<TextInputSubmitEditingEventData>) => {
        ...
        await handleSearchKupon(setKuponData, setSearchKuponQuery, searchKuponQuery);
    };
    return (
        <View>
            <View style={styles.header}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 10, alignItems: 'center' }}>
                    ...
                    <Text style={{ fontSize: 25, fontFamily: Poppins.ExtraBold, color: Color.border }}>list kupon</Text>
                    <TouchableOpacity
                        style={styles.TouchableBack}
                        onPress={() => handleRefreshList(setIsRefreshing, refreshAnimation, refreshRotation,
...
export default HeaderKuponPage
